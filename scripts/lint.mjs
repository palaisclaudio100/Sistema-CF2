...
for (const file of files.filter(file => file.endsWith('.mjs') && path.basename(file) !== 'lint.mjs')) { const text = fs.readFileSync(file, 'utf8'); if (text.includes('drive.google.com') || text.includes('googleapis') || text.includes('fs.watch')) errors.push(`${file}: Etapa 0 forbids production integration or watcher`); }
