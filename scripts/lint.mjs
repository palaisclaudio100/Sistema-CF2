...
for (const file of files.filter(file => file.endsWith('.mjs') && path.basename(file) !== 'lint.mjs')) { const text = fs.readFileSync(file, 'utf8'); const approvedIntegration=['stage5-drive-bridge.mjs','stage5-cloud-gate.test.mjs','google-role-gateway.mjs','canon-gateway.mjs'].includes(path.basename(file)); if (!approvedIntegration&&(text.includes('drive.google.com') || text.includes('googleapis') || text.includes('fs.watch'))) errors.push(`${file}: production integration or watcher is forbidden outside the explicit allowlist`); }
