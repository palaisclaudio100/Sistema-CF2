...
try{
  await store.migrate();
  const service=new RoleEnrollmentService(store,{baseUrl,signingSecret:process.env.CF2_GOOGLE_OAUTH_CLIENT_SECRET}),pair=await service.createPair();
  process.stdout.write(`${JSON.stringify({ttl_seconds:ENROLLMENT_TTL_SECONDS,enrollments:Object.fromEntries(pair.map(item=>[item.enrollment_id,item.url]))})}\n`);
