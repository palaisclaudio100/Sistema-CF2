...
export class OrchestrationApi{
  constructor(transport,canon,{pool,workerKeys={},clock=()=>Date.now(),onClaimShape=()=>{}}={}){this.transport=transport;this.canon=canon;this.pool=pool;this.keys=workerKeys;this.clock=clock;this.onClaimShape=onClaimShape;}
  ...
  async worker(principal,operation,args){
    if(principal?.actor_id==='ACTOR:CODEX'&&operation==='claim')this.onClaimShape({point:'P3_WORKER_ENTRY',actor_id:principal.actor_id,operation,args});
