...
export const CLAUDE_CODE_ENROLLMENT_SPEC=Object.freeze({enrollment_id:'ENROLLMENT:CLAUDE_CODE',actor_id:'ACTOR:CLAUDE_CODE'});
...
export class RoleEnrollmentService{
  ...
  async createClaudeCodeEnrollment(){
    const spec=CLAUDE_CODE_ENROLLMENT_SPEC,client=await this.pool.connect(),createdAt=new Date(this.clock()),expiresAt=new Date(this.clock()+ENROLLMENT_TTL_SECONDS*1000),token=randomToken();
    try{
      await client.query('BEGIN');
      const existing=await client.query('SELECT enrollment_id FROM role_gateway_enrollments WHERE enrollment_id=$1 FOR UPDATE',[spec.enrollment_id]);
      if(existing.rowCount)throw new Error('ENROLLMENT_ALREADY_EXISTS');
