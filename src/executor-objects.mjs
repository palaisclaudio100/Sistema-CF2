...
export class ExecutorObjects{
  ...
  async writeValidated(context,fence){
    ...
    let lockPath,lockMark;try{
      const o=await this.object(actor_id,step.object_id,true);await fs.mkdir(this.stateRoot,{recursive:true});
      ...
      try{await fs.writeFile(lockPath,lockMark,{flag:'wx'});}catch(e){if(e.code!=='EEXIST')throw e;const held=JSON.parse(await fs.readFile(lockPath,'utf8'));let alive=true;try{process.kill(held.pid,0);}catch(check){if(check.code==='ESRCH')alive=false;}if(alive){lockMark=null;throw new Error('OBJECT_BUSY');}await fs.unlink(lockPath);await fs.writeFile(lockPath,lockMark,{flag:'wx'});}
      const failurePath=path.join(this.stateRoot,sha(step.object_id)+'.verification-failures');let failures=0;try{failures=Number(await fs.readFile(failurePath,'utf8'));}catch(e){if(e.code!=='ENOENT')throw e;}if(failures>=2)throw new Error('MATERIAL_VERIFICATION_LOCKED');const receiptPath=path.join(this.stateRoot,sha(message_id)+'.json');let old;
      ...
      const backup=path.join(this.stateRoot,sha(message_id)+'.before');if(before.content!==null)await fs.writeFile(backup,await fs.readFile(o.file),{flag:'wx'}).catch(async e=>{if(e.code!=='EEXIST'||sha(await fs.readFile(backup))!==before.sha256)throw e;});
  ...
  async runRegistered(actor,id,fence){const c=this.commands.get(id);if(!c||!c.actors?.includes(actor)||actor==='ACTOR:GABY_CHAT')throw new Error('COMMAND_SCOPE_DENIED');
    ...
    return{command_id:id,actor_id:actor,exit_code:r.exit_code,stdout_sha256:sha(r.stdout),stdout:r.stdout.slice(0,8000),verified_at:new Date().toISOString()};
  }
}
