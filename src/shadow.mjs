...
export function compareLogical(cf1={},cf2={}) { const left=asMap(cf1),right=asMap(cf2),keys=new Set([...left.keys(),...right.keys()]),discrepancies=[]; for(const key of [...keys].sort()) {const a=left.get(key),b=right.get(key);if(a===undefined||b===undefined)discrepancies.push({object:key,property:'logical_state',CF1:a??EMPTY,CF2:b??EMPTY,classification:'UNKNOWN',action_proposed:'obtain_source_evidence'});else if(fingerprint(a)!==fingerprint(b))discrepancies.push({object:key,property:'logical_state',CF1:a,CF2:b,classification:'CONFLICT',action_proposed:'review_source_evidence'});} for(const collection of BASELINE_COLLECTIONS) {const a=cf1?.coverage?.[collection]??'UNKNOWN',b=cf2?.coverage?.[collection]??'UNKNOWN';if(a==='AUTHORITATIVE_CURRENT'&&b==='AUTHORITATIVE_CURRENT')continue;const classification=[a,b].includes('CONFLICT')?'CONFLICT':[a,b].includes('CANDIDATE')?'CANDIDATE':'UNKNOWN';discrepancies.push({object:`coverage:${collection}`,property:'source_coverage',CF1:{status:a},CF2:{status:b},classification,action_proposed:'obtain_typed_source_evidence'});} const count=classification=>discrepancies.filter(item=>item.classification===classification).length; return Object.freeze({kind:'CF1_CF2_LOGICAL_COMPARISON',no_side_effect:true,discrepancies,conflict:count('CONFLICT'),unknown:count('UNKNOWN'),candidate:count('CANDIDATE'),result:discrepancies.length?'DIFFERENCES_DETECTED':'EQUAL'}); }

/** Shadow is observation-only: it cannot receive adapters or execute external effects. */
export class ShadowRuntime {
  constructor({at=now,minimumSoakMs=SHADOW_MINIMUM_SOAK_MS,store=null,sessionId=`SHADOW:${crypto.randomUUID()}`,baselineDigest='UNSEALED'}={}) { if(!Number.isSafeInteger(minimumSoakMs)||minimumSoakMs<1)fail('INVALID_SOAK_DURATION');this.at=at;this.minimumSoakMs=minimumSoakMs;this.store=store;this.sessionId=sessionId;this.baselineDigest=baselineDigest;this.started_at=null;this.observations=[];this.would_do=[];this.keys=new Set(); }
  start() { if(!this.started_at) { const localStarted=this.at(); const session=this.store?.startShadowSession({session_id:this.sessionId,baseline_digest:this.baselineDigest,started_at:localStarted,minimum_soak_ms:this.minimumSoakMs}); this.started_at=session?.started_at??localStarted; } return {mode:'SHADOW_READ',started_at:this.started_at,no_side_effect:true,session_id:this.sessionId}; }
  observe({baseline,current,label='read-only'}={}) { if(!this.started_at)fail('SHADOW_NOT_STARTED');const comparison=compareLogical(baseline,current),observation=Object.freeze({observation_id:`SHADOW_OBSERVATION:${crypto.randomUUID()}`,session_id:this.sessionId,observed_at:this.at(),label,comparison,mode:'SHADOW_READ',no_side_effect:true});this.observations.push(observation);this.store?.recordShadowObservation({observation_id:observation.observation_id,session_id:this.sessionId,observed_at:observation.observed_at,body:observation});return observation; }
  ...
  report() { const soak=this.soakStatus(),observations=this.store?this.store.listShadowObservations(this.sessionId):this.observations;return Object.freeze({kind:'SHADOW_SOAK_REPORT',session_id:this.sessionId,started_at:this.started_at,observations,would_do:this.would_do,duration_ms:soak.elapsed_ms??0,minimum_soak_ms:this.minimumSoakMs,soak_result:soak.result,no_side_effect:true}); }
}
