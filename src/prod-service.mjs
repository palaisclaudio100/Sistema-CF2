...
const shape=value=>({present:value!==undefined,type:value===null?'null':Array.isArray(value)?'array':typeof value,is_array:Array.isArray(value),keys:value&&typeof value==='object'&&!Array.isArray(value)?Object.keys(value).sort():[]});
const claimTrace=entry=>console.error(JSON.stringify({event:`CF2_CLAIM_${entry.point}`,actor_id:entry.actor_id,operation:entry.operation,args_shape:shape(entry.args),release_id:config.release_id,pid:process.pid,timestamp:new Date().toISOString()}));
const orchestration=new OrchestrationApi(transport,canon,{pool:store.pool,workerKeys:JSON.parse(process.env.CF2_ORCHESTRATION_WORKER_KEYS??'{}'),onClaimShape:claimTrace});
...
const server=http.createServer(async(request,response)=>{
  ...
  if(url.pathname==='/internal/actor-runtime'&&request.method==='POST'){
    ...
    try{const body=await readBody(request);if(Object.keys(body).some(k=>!['operation','args'].includes(k)))throw new Error('INVALID_SCHEMA');if(principal.actor_id==='ACTOR:CODEX'&&body.operation==='claim')console.error(JSON.stringify({event:'CF2_CLAIM_P1_PARSED_BODY',actor_id:principal.actor_id,operation:body.operation,body_shape:shape(body),body_keys:Object.keys(body).sort(),raw_args_shape:shape(body.args),release_id:config.release_id,pid:process.pid,timestamp:new Date().toISOString()}));const effectiveArgs=body.args;if(principal.actor_id==='ACTOR:CODEX'&&body.operation==='claim')claimTrace({point:'P2_WORKER_CALL',actor_id:principal.actor_id,operation:body.operation,args:effectiveArgs});return send(response,200,{result:'PASS',data:await orchestration.worker(principal,body.operation,effectiveArgs)});}catch(error){return send(response,409,{result:'FAIL_CLOSED',error_code:['ROLE_FORBIDDEN','INVALID_SCHEMA','EXECUTOR_SCOPE_DENIED','LEASE_REJECTED','CANON_NOT_VERIFIED','THREAD_ALREADY_EXISTS','THREAD_UNKNOWN','THREAD_TERMINAL','THREAD_NOT_COMPLETE','IDEMPOTENCY_CONFLICT','INVALID_STATE'].includes(error.message)?error.message:'FAIL_CLOSED'});}
  }
