...
export class GoogleDriveClient {
  ...
  metadata(id){return this.api(`/drive/v3/files/${encodeURIComponent(id)}?fields=id,name,mimeType,parents,modifiedTime,version,headRevisionId,lastModifyingUser(permissionId)&supportsAllDrives=true`);}
...
export class Stage5DriveBridge {
  constructor({drive,roleInterface,gateInstanceId=crypto.randomUUID(),clock=()=>Date.now(),logger=console}){this.drive=drive;this.roles=roleInterface;this.gateInstanceId=gateInstanceId;this.clock=clock;this.logger=logger;this.channel=null;this.files=null;this.baseline=null;this.principalPermissionId=null;this.seen=new Set();this.nonces=new Set();this.audit=[];this.envelopes=null;}
  async bootstrap({folderId,requestId,responseId,webhookUrl,watchHours=12}){
    ...
    const requestMeta=await this.drive.metadata(files.requestId);this.files=files;this.baseline={file_id:requestMeta.id,name:requestMeta.name,modified_time:requestMeta.modifiedTime??null,version:requestMeta.version,headRevisionId:requestMeta.headRevisionId??null};this.envelopes=this.#envelopes();
  ...
  #log(event,extra={}){this.logger.info?.(JSON.stringify({event,timestamp:iso(),gate_instance_id:this.gateInstanceId,request_file_id:this.files?.requestId??null,...extra}));}
  ...
  async webhook(headers){
    if(!this.channel||!this.files)return {http:503,result:{result:'NOT_READY'}};this.#log('WEBHOOK_RECEIVED');const channel=validateChannel(headers,this.channel);if(!channel.ok){this.#log(channel.code==='INITIAL_SYNC_IGNORED'?'WATCH_SYNC_IGNORED':'WEBHOOK_REJECTED',{result:channel.code});return {http:channel.ignored?204:403,result:{result:channel.code}};}
    const meta=await this.drive.metadata(this.files.requestId);this.#log('METADATA_READBACK',{result:'PASS',modified_time:meta.modifiedTime??null});if(meta.id!==this.files.requestId||meta.mimeType!=='text/plain'||!meta.parents?.includes(this.files.folderId))return this.#respond(null,'REMOTE_READ_INVALID');
    if(meta.id===this.baseline.file_id&&meta.name===this.baseline.name&&meta.modifiedTime===this.baseline.modified_time){this.#log('BASELINE_IGNORED');return {http:204,result:{result:'BASELINE_IGNORED'}};
    }
    if(!meta.lastModifyingUser?.permissionId||meta.lastModifyingUser.permissionId!==this.principalPermissionId){this.#log('ACTOR_BINDING_FAIL',{result:'ACTOR_BINDING_UNPROVEN'});return this.#respond(null,'ACTOR_BINDING_UNPROVEN');}this.#log('ACTOR_BINDING_PASS');
    const parsed=parseMetadataEnvelope(meta.name,this.gateInstanceId);if(!parsed.ok)return this.#respond(null,parsed.code);const req=parsed.value;this.#log('ENVELOPE_ACCEPTED',{operation:req.operation,request_id:req.request_id});const transportKey=`${this.files.requestId}:${meta.name}:${meta.modifiedTime??'NO_MODIFIED_TIME'}:${req.request_id}:${req.nonce}`;
    ...
    let response;if(req.operation==='READ')response=this.roles.client('dev-credential-dga').request('READ_SNAPSHOT');else if(req.operation==='STATUS')response=this.roles.client('dev-credential-dga').request('READ_COMMAND_STATUS',{command_id:COMMAND_ID});else response=this.roles.client('dev-credential-dga').submitCommand(this.#command());this.#log('ROLE_INTERFACE_EXECUTED',{operation:req.operation,request_id:req.request_id,result:response.ok?'PASS':response.reason_code});
  ...
  async #respond(req,result,extra={}){const response={request_id:req?.request_id??null,gate_instance_id:this.gateInstanceId,actor_id:'ACTOR:DIEGO',actor_role:'DGA',actor_binding_scope:'STAGING_CHANNEL_ONLY',operation:req?.operation??null,result,state_version:extra.response?.state_version??this.roles.client('dev-credential-dga').request('HEALTH').state_version,command_id:['COMMAND','REPLAY','STATUS'].includes(req?.operation)?COMMAND_ID:null,status:extra.response?.data?.command_status??extra.response?.data?.status??null,task_ref:extra.response?.data?.task_ref??null,audit_ref:req?.request_id?`AUDIT:${hash(req.request_id).slice(0,16)}`:null,processed_at:iso(),error_code:result==='PASS'?null:result,replay:Boolean(extra.replay)};this.#log('RESPONSE_WRITE_ATTEMPT',{operation:req?.operation??null,request_id:req?.request_id??null,result});try{await this.drive.update(this.files.responseId,Buffer.from(JSON.stringify(response,null,2)+'\n'));this.#log('RESPONSE_WRITE_PASS',{operation:req?.operation??null,request_id:req?.request_id??null,result});}catch(error){this.#log('RESPONSE_WRITE_FAIL',{operation:req?.operation??null,request_id:req?.request_id??null,result:'RESPONSE_WRITE_FAILED'});throw error;}this.audit.push({...response,channel_id_fingerprint:this.channel?hash(this.channel.channelId).slice(0,16):null,request_revision:extra.revision??null});return {http:200,result:response};}
}
