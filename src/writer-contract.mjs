...
const verificationObject={type:'object',additionalProperties:false,required:['id','type','status','created_at','updated_at','basis_ref','subject_id','attribute','value','class','verified_at','verified_by','evidence_ref'],properties:{...commonObject,type:{const:'VERIFICATION'},subject_id:{type:'string',minLength:1},attribute:{type:'string',minLength:1},value:{},class:{enum:VERIFICATION_CLASSES},verified_at:{type:'string',format:'date-time'},verified_by:{type:'string',description:'Accepted for compatibility and overwritten with the authenticated actor_id.'},evidence_ref:{type:'string',minLength:1},valid_until:{type:'string',format:'date-time'},invalidation_rule:{type:'string',minLength:1}}};
...
export function validateWriterToolArgs(tool,args,allowedRoles=ACTING_ROLES){
  ...
  if(tool==='submit_verification'&&command.command_type==='RECORD_VERIFICATION'){
    const payload=command.payload,object=payload?.object;
    if(!exactKeys(payload,new Set(['object']))||!exactKeys(object,verificationKeys)||!commonValid(object)||object.type!=='VERIFICATION'||!string(object.subject_id)||!string(object.attribute)||!('value' in object)||!VERIFICATION_CLASSES.includes(object.class)||!timestamp(object.verified_at)||!string(object.verified_by)||!string(object.evidence_ref))return false;
