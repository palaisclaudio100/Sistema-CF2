...
export class CoreStore {
  ...
  enqueueDevJob({job_type,payload={},dedupe_key,event_id=null,at=now()}) { const job_id=`JOB:${crypto.randomUUID()}`; this.db.prepare('INSERT OR IGNORE INTO jobs(job_id,event_id,job_type,payload,status,attempt,dedupe_key,next_attempt_at) VALUES(?,?,?,?,?,?,?,?)').run(job_id,event_id,job_type,json(payload),'READY',0,dedupe_key,at); const row=this.db.prepare('SELECT * FROM jobs WHERE dedupe_key=?').get(dedupe_key); return row; }
  dispatchPendingOutbox(at=now()) { const rows=this.db.prepare("SELECT * FROM outbox WHERE status='PENDING' ORDER BY created_at").all(); this.db.exec('BEGIN IMMEDIATE'); try { for (const row of rows) { this.enqueueDevJob({job_type:'OUTBOX_EVENT',payload:parse(row.payload),dedupe_key:`EVENT:${row.event_id}`,event_id:row.event_id,at}); this.db.prepare("UPDATE outbox SET status='DISPATCHED' WHERE event_id=?").run(row.event_id); } this.db.exec('COMMIT'); return rows.length; } catch(error) { this.db.exec('ROLLBACK'); throw error; } }
  recoverExpiredLeases(at=now()) { const rows=this.db.prepare("SELECT * FROM jobs WHERE status IN ('CLAIMED','RUNNING') AND lease_until<=?").all(at); for (const row of rows) { this.db.prepare("UPDATE jobs SET status='RETRY_WAIT',worker_id=NULL,claimed_at=NULL,lease_until=NULL,next_attempt_at=?,last_error=? WHERE job_id=?").run(at,'LEASE_EXPIRED',row.job_id); this.#jobAttempt(row.job_id,row.attempt,'LEASE_EXPIRED',row.worker_id,{reason:'LEASE_EXPIRED'},at); this.#metric('leases_expired',at); } return rows.length; }
  claimNextJob(worker_id,{at=now(),lease_ms=30000}={}) { const row=this.db.prepare("SELECT * FROM jobs WHERE status='READY' OR (status='RETRY_WAIT' AND next_attempt_at<=?) ORDER BY COALESCE(next_attempt_at,''),job_id LIMIT 1").get(at); if (!row) return null; const lease_until=new Date(new Date(at).getTime()+lease_ms).toISOString(); const changed=this.db.prepare("UPDATE jobs SET status='CLAIMED',worker_id=?,claimed_at=?,lease_until=?,attempt=attempt+1 WHERE job_id=? AND (status='READY' OR (status='RETRY_WAIT' AND next_attempt_at<=?))").run(worker_id,at,lease_until,row.job_id,at).changes; if (!changed) return null; const claimed=this.db.prepare('SELECT * FROM jobs WHERE job_id=?').get(row.job_id); this.#jobAttempt(claimed.job_id,claimed.attempt,'CLAIMED',worker_id,{lease_until},at); return claimed; }
  ...
  retryJob(job_id,worker_id,error,{at=now(),max_attempts=3,base_delay_ms=1000}={}) { const job=this.db.prepare('SELECT * FROM jobs WHERE job_id=?').get(job_id); if (!job || job.worker_id!==worker_id || job.status!=='RUNNING') throw new CF2Error('FAIL_CLOSED'); const terminal=job.attempt>=max_attempts; const delay=Math.min(base_delay_ms*(2**Math.max(0,job.attempt-1)),60000); const next=new Date(new Date(at).getTime()+delay).toISOString(); this.db.prepare('UPDATE jobs SET status=?,lease_until=NULL,next_attempt_at=?,last_error=? WHERE job_id=?').run(terminal?'FAILED':'RETRY_WAIT',terminal?null:next,String(error),job_id); this.#jobAttempt(job_id,job.attempt,terminal?'FAILED':'RETRY_WAIT',worker_id,{error:String(error),next_attempt_at:terminal?null:next},at); this.#metric(terminal?'jobs_errors':'jobs_retries',at); return this.db.prepare('SELECT * FROM jobs WHERE job_id=?').get(job_id); }
  listDueVolatile(at=now()) { return this.#allObjects('VERIFICATION','CURRENT').filter(v=>v.class==='VOLÁTIL' && v.valid_until<=at); }
  workerHealth(at=now()) { const byStatus={}; for (const row of this.db.prepare('SELECT status,COUNT(*) AS count FROM jobs GROUP BY status').all()) byStatus[row.status]=row.count; const metrics=Object.fromEntries(this.db.prepare('SELECT metric_key,metric_value FROM worker_metrics').all().map(x=>[x.metric_key,x.metric_value])); const last=this.db.prepare('SELECT occurred_at FROM job_attempts ORDER BY occurred_at DESC LIMIT 1').get(); const last_errors=this.db.prepare("SELECT job_id,last_error FROM jobs WHERE last_error IS NOT NULL ORDER BY job_id DESC LIMIT 10").all(); const recent_events=this.db.prepare("SELECT event_id,event_type,status,created_at FROM outbox WHERE status='DISPATCHED' ORDER BY created_at DESC LIMIT 10").all(); const since=last ? Math.max(0,new Date(at).getTime()-new Date(last.occurred_at).getTime()) : null; return {worker_health:'DEV_MANUAL',jobs:byStatus,outbox_pending:this.db.prepare("SELECT COUNT(*) AS count FROM outbox WHERE status='PENDING'").get().count,expired_leases:this.db.prepare("SELECT COUNT(*) AS count FROM jobs WHERE status IN ('CLAIMED','RUNNING') AND lease_until<=?").get(at).count,metrics,last_work_at:last?.occurred_at ?? null,time_since_last_work_ms:since,last_errors,recent_events}; }
