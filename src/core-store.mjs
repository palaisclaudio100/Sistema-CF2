...
export class CoreStore {
  ...
  #migrate() {
    this.db.exec(`PRAGMA foreign_keys=ON; PRAGMA journal_mode=DELETE;
      ...
      CREATE TABLE IF NOT EXISTS shadow_sessions (session_id TEXT PRIMARY KEY, baseline_digest TEXT NOT NULL, started_at TEXT NOT NULL, minimum_soak_ms INTEGER NOT NULL, mode TEXT NOT NULL);
      CREATE TABLE IF NOT EXISTS shadow_observations (observation_id TEXT PRIMARY KEY, session_id TEXT NOT NULL, observed_at TEXT NOT NULL, body TEXT NOT NULL, UNIQUE(session_id, observed_at, body));
      CREATE INDEX IF NOT EXISTS shadow_observations_session ON shadow_observations(session_id, observed_at);`);
  ...
  registerStagingMapping(mapping) { const fields=['mapping_id','artifact_id','generation_id','A_path','C_file_id','replication_mode','byte_preserving','environment','status','created_at','updated_at','basis_ref']; for (const field of fields) if (!(field in mapping)) throw new CF2Error('INVALID_ID',`Missing mapping ${field}`); if (mapping.environment!=='STAGING'||mapping.status!=='AUTHORIZED'||!mapping.C_file_id||mapping.replication_mode!=='A_TO_C_EXACT_ID'||mapping.byte_preserving!==true) throw new CF2Error('FAIL_CLOSED'); const existing=this.db.prepare('SELECT body FROM mappings WHERE artifact_id=? AND generation=?').get(mapping.artifact_id,mapping.generation_id); if(existing&&parse(existing.body).C_file_id!==mapping.C_file_id) throw new CF2Error('MAPPING_ID_CHANGE_REQUIRES_NEW_GENERATION'); this.db.prepare('INSERT INTO mappings VALUES(?,?,?,?,?,?,?,?,?,?)').run(mapping.mapping_id,mapping.artifact_id,mapping.generation_id,mapping.A_path,mapping.C_file_id,mapping.environment,mapping.status,json(mapping),mapping.created_at,mapping.updated_at); return mapping; }
  ...
  startShadowSession({session_id,baseline_digest,started_at,minimum_soak_ms,mode='SHADOW_READ'}) { for(const field of ['session_id','baseline_digest','started_at','minimum_soak_ms']) if (!({session_id,baseline_digest,started_at,minimum_soak_ms}[field])) throw new CF2Error('MISSING_EVIDENCE'); if (!Number.isSafeInteger(minimum_soak_ms)||minimum_soak_ms<1||mode!=='SHADOW_READ') throw new CF2Error('FAIL_CLOSED'); this.db.prepare('INSERT OR IGNORE INTO shadow_sessions VALUES(?,?,?,?,?)').run(session_id,baseline_digest,started_at,minimum_soak_ms,mode); const row=this.db.prepare('SELECT * FROM shadow_sessions WHERE session_id=?').get(session_id); if(row.baseline_digest!==baseline_digest||row.minimum_soak_ms!==minimum_soak_ms) throw new CF2Error('FAIL_CLOSED'); return row; }
  recordShadowObservation({observation_id=`SHADOW_OBSERVATION:${crypto.randomUUID()}`,session_id,observed_at,body}) { if(!session_id||!observed_at||!body||body.no_side_effect!==true) throw new CF2Error('MISSING_EVIDENCE'); if(!this.db.prepare('SELECT 1 FROM shadow_sessions WHERE session_id=?').get(session_id)) throw new CF2Error('UNKNOWN'); this.db.prepare('INSERT OR IGNORE INTO shadow_observations VALUES(?,?,?,?)').run(observation_id,session_id,observed_at,json(body)); return this.db.prepare('SELECT * FROM shadow_observations WHERE observation_id=?').get(observation_id) ? body : null; }
  getShadowSession(session_id) { return this.db.prepare('SELECT * FROM shadow_sessions WHERE session_id=?').get(session_id) ?? null; }
  listShadowObservations(session_id) { return this.db.prepare('SELECT body FROM shadow_observations WHERE session_id=? ORDER BY observed_at,observation_id').all(session_id).map(row=>parse(row.body)); }
