...
const endpointByActor={
  ...
  'ACTOR:CLAUDE_CODE':'/mcp/claude-code'
};
...
test('MULTI-02C Claude Code publishes TASK tools without verification',async()=>{const tools=await toolsListFor('ACTOR:CLAUDE_CODE');assert.deepEqual(tools.map(tool=>tool.name),['get_my_tasks','get_task','submit_task_command','get_status']);for(const roles of writerEnums(tools))assert.deepEqual(roles,['CLAUDE_CODE']);});
