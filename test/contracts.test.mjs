...
test('T05 volatile verification without validity fails closed', () => { const invalid = {...byId('VERIFICATION:YT:AI'), id:'VERIFICATION:VOLATILE', class:'VOLÁTIL'}; validateObject(invalid, 'volatile'); assert.ok(errors.some(e => e.includes('STALE_VERIFICATION'))); errors.length = 0; });
